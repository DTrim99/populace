...
def test_uk_population_targets_shape_order_and_registry_accounting() -> None:
    ...
    assert len(resource["targets"]) == 305
    ...
    assert len(registry_scope) == 254
    assert len(profile_scope) == 51
    assert target_ids[:254] == registry_scope
    assert target_ids[254:] == profile_scope
    ...
    assert len(unmapped_declarations) == 60
