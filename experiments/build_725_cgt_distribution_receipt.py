...
def _frame(path: Path) -> pd.DataFrame:
    ...
    weights = pd.Series(
        household["household_weight"].to_numpy(), index=household["household_id"]
    )
    regions = pd.Series(
    ...
    out = pd.DataFrame(
        {
            "gains": pd.to_numeric(person["capital_gains"], errors="raise").to_numpy(
                dtype=float
            ),
...
def _summaries(frame: pd.DataFrame) -> dict[str, pd.DataFrame]:
    ...
    size = _by(
        frame,
        _band(gains, HMRC_CGT_SIZE_BAND_LOWER_BOUNDS),
        HMRC_CGT_SIZE_BAND_LOWER_BOUNDS,
    )
    ...
    region["published_people_individuals_basis"] = [
        r.taxpayers * share_people for r in conditioning.regions
    ]
    region["published_gains_individuals_basis"] = [
        r.gains * share_gains for r in conditioning.regions
    ]
            ...
            joint.append(
                {
                    ...
                    "people": float(liable["weight"][mask].sum()),
                    "gains": float(
                        (liable["weight"][mask] * liable["gains"][mask]).sum()
                    ),
                }
            )
...
def _headline(frame: pd.DataFrame) -> dict[str, float]:
    ...
    return {
        ...
        "share_65_plus_of_taxpayers": float(
            liable["weight"][older].sum() / liable["weight"].sum()
        ),
        "share_65_plus_of_gains": float(
            (liable["weight"] * liable["gains"])[older].sum()
            / (liable["weight"] * liable["gains"]).sum()
        ),
        "share_65_plus_of_2m_plus_gainers": float(
            top["weight"][older_top].sum() / top["weight"].sum()
        )
        if len(top)
        else float("nan"),
        "share_65_plus_of_2m_plus_gains": float(
            (top["weight"] * top["gains"])[older_top].sum()
            / (top["weight"] * top["gains"]).sum()
        )
        if len(top)
        else float("nan"),
...
def _asset_types(frame: pd.DataFrame) -> pd.DataFrame | None:
    ...
    table.loc[CGT_ASSET_TYPE_RESIDENTIAL, "published_people_individuals_basis"] = (
        facts.residential_taxpayers_individuals_basis
    )
    table.loc[CGT_ASSET_TYPE_RESIDENTIAL, "published_gains_individuals_basis"] = (
        facts.residential_gains_individuals_basis
    )
    ...
    for name in CGT_ASSET_TYPE_NON_RESIDENTIAL_TYPES:
        table.loc[name, "achieved_gains_share"] = (
            table.loc[name, "gains"] / non_res if non_res else float("nan")
        )
...
def _stage_evidence(sidecar: Path) -> dict[str, object]:
    ...
    return {
        ...
        for stage in ("hmrc_cgt_gains_spine", "hmrc_cgt_asset_type_spine")
...
def main() -> int:
    ...
    for name, frame in frames.items():
        ...
        receipt[f"{name}_stage_evidence"] = _stage_evidence(
            args.__dict__[name].with_suffix(".build.json")
        )
    (args.out / f"receipt{args.label}.json").write_text(
