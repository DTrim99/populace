...
O = os.path.expanduser("~/PolicyEngine/_recovered/scratch-backup/893/chronicle-export")
PINNED = os.path.expanduser("~/PolicyEngine/_buildh-runtime/inputs/consumer_facts_buildn_v9_4.jsonl")
years = sys.argv[1:]
def ns(row): return ((row.get("layout") or {}).get("record_set_id") or "").split(".")[0]
keep_ns, pinned_rs = set(), set()
with open(PINNED) as f:
    for line in f:
        if line.strip():
            r = json.loads(line); keep_ns.add(ns(r)); pinned_rs.add((r.get("layout") or {}).get("record_set_id"))
rows, conflicts, seen_in = {}, [], collections.defaultdict(set)
for y in years:
    n = 0
    with open(f"{O}/ledger-us-{y}/consumer_facts.jsonl") as f:
        for line in f:
            if not line.strip(): continue
            r = json.loads(line)
            if ns(r) not in keep_ns: continue
            k = r["aggregate_fact_key"]; n += 1; seen_in[k].add(y)
            if k in rows:
                if rows[k][0].get("value") != r.get("value"): conflicts.append((k, rows[k][0].get("value"), r.get("value")))
                continue
...
out = f"{O}/us-merged-{'-'.join(years)}.jsonl"
h = hashlib.sha256()
with open(out, "w") as f:
    for k in sorted(rows):
        f.write(rows[k][1]); h.update(rows[k][1].encode())
new_rs = {(r.get("layout") or {}).get("record_set_id") for r, _ in rows.values()}
print("wrote", out, "sha256", h.hexdigest()[:16], "| record sets:", len(new_rs), "| pinned sets missing:", len(pinned_rs - new_rs))
